...
import { db } from "../db";
import { projects, stations, boreholes, structuralMeasurements } from "../db/schema";
...
// 2. GET /api/projects/:projectId/export/csv - CSV Tabular Data Export with Formula Injection Sanitization
router.get(
    ...
    try {
      ...
      if (target === "boreholes") {
        ...
        const rows = bList.map((b) =>
          [
            ...
            sanitizeCsvField(b.boreholeNumber),
            sanitizeCsvField(b.name),
            sanitizeCsvField(b.latitude),
            sanitizeCsvField(b.longitude),
            sanitizeCsvField(b.elevation),
            sanitizeCsvField(b.totalDepth),
            sanitizeCsvField(b.staticWaterLevel),
            sanitizeCsvField(b.dynamicWaterLevel),
            sanitizeCsvField(b.dischargeRate),
            sanitizeCsvField(b.aquiferType),
          ].join(",")
        );
      ...
      stList.forEach((st) => {
        st.rockSamples.forEach((r) => {
          rows.push(
            [
              sanitizeCsvField(st.code),
              sanitizeCsvField(st.name),
              sanitizeCsvField(r.sampleBagId),
              sanitizeCsvField(r.probableRock),
              sanitizeCsvField(r.grainSize),
              sanitizeCsvField(r.texture),
              sanitizeCsvField(r.maficPercent),
              sanitizeCsvField(r.felsicPercent),
              sanitizeCsvField(st.latitude),
              sanitizeCsvField(st.longitude),
