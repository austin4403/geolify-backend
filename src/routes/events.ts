...
router.get("/projects/:projectId/events", (req: Request, res: Response) => {
  ...
  // Register client to this project's channel
  if (!projectClients.has(projectId)) {
    projectClients.set(projectId, new Set());
  }
  projectClients.get(projectId)!.add(res);
  ...
  res.write(
    `event: connected\ndata: ${JSON.stringify({
      ...
      activeTeammates: projectClients.get(projectId)!.size,
    })}\n\n`
  );
...
router.post("/projects/:projectId/ping-location", async (req: Request, res: Response) => {
  try {
    const rawId = Array.isArray(req.params.projectId) ? req.params.projectId[0] : req.params.projectId;
    const projectId = parseInt(rawId, 10);
...
router.get("/projects/:projectId/live-locations", async (req: Request, res: Response) => {
  try {
    const rawId = Array.isArray(req.params.projectId) ? req.params.projectId[0] : req.params.projectId;
    const projectId = parseInt(rawId, 10);
